...
class CVACTDatasetTrain(Dataset):
    def __init__(
        ...
        for idx in train_ids.squeeze():
            ...
            if not os.path.exists(
                f"{self.data_folder}/{grd_path}"
            ) or not os.path.exists(f"{self.data_folder}/{sat_path}"):
    ...
    def __getitem__(self, index):
        ...
        query_img = cv2.imread(
            f"{self.data_folder}/ANU_data_small/streetview/{idx}_grdView.jpg"
        )
        query_img = cv2.cvtColor(query_img, cv2.COLOR_BGR2RGB)

        # load reference -> satellite image
        reference_img = cv2.imread(
            f"{self.data_folder}/ANU_data_small/satview_polish/{idx}_satView_polish.jpg"
        )
...
class CVACTDatasetEval(Dataset):
    def __init__(
        ...
        if split != "train" and split != "val":
            raise ValueError(
                "Invalid 'split' parameter. 'split' must be 'train' or 'val'"
            )
...
class CVACTDatasetTest(Dataset):
    def __init__(
        ...
        files_sat = glob(
            f"{self.data_folder}/ANU_data_test/satview_polish/*_satView_polish.jpg"
        )
        files_ground = glob(
            f"{self.data_folder}/ANU_data_test/streetview/*_grdView.jpg"
        )
