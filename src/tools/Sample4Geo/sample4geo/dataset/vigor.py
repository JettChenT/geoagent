...
class VigorDatasetTrain(Dataset):
    def __init__(
        ...
        for city in self.cities:
            df_tmp = pd.read_csv(
                f"{data_folder}/splits/{city}/satellite_list.txt",
                header=None,
                delim_whitespace=True,
            )
            df_tmp = df_tmp.rename(columns={0: "sat"})
            df_tmp["path"] = df_tmp.apply(
                lambda x: f"{data_folder}/satellite/{city}/{x.sat}", axis=1
            )
        ...
        for city in self.cities:
            if same_area:
                df_tmp = pd.read_csv(
                    f"{data_folder}/splits/{city}/same_area_balanced_train.txt",
                    header=None,
                    delim_whitespace=True,
                )
            else:
                df_tmp = pd.read_csv(
                    f"{data_folder}/splits/{city}/pano_label_balanced.txt",
                    header=None,
                    delim_whitespace=True,
                )

            df_tmp = df_tmp.loc[:, [0, 1, 4, 7, 10]].rename(
                columns={
                    0: "ground",
                    1: "sat",
                    4: "sat_np1",
            ...
            df_tmp["path_ground"] = df_tmp.apply(
                lambda x: f"{data_folder}/ground/{city}/{x.ground}", axis=1
            )
            df_tmp["path_sat"] = df_tmp.apply(
                lambda x: f"{data_folder}/satellite/{city}/{x.sat}", axis=1
            )
        ...
        self.idx2ground_path = dict(
            zip(self.df_ground.index, self.df_ground.path_ground)
        )
    ...
    def shuffle(self, sim_dict=None, neighbour_select=8, neighbour_range=16):
            ...
            if len(pair_pool) > 0:
                ...
                if (
                    ...
                    if (
                        ...
                        near_similarity = copy.deepcopy(
                            similarity_pool[idx][:neighbour_range]
                        )
        ...
        print(
            "First Element ID: {} - Last Element ID: {}".format(
                self.samples[0][1], self.samples[-1][1]
            )
        )
...
class VigorDatasetEval(Dataset):
    def __init__(
        ...
        for city in self.cities:
            if same_area:
                df_tmp = pd.read_csv(
                    f"{data_folder}/splits/{city}/same_area_balanced_{split}.txt",
                    header=None,
                    delim_whitespace=True,
                )
            else:
            ...
            df_tmp["path_sat_np1"] = df_tmp.apply(
                lambda x: f"{data_folder}/satellite/{city}/{x.sat_np1}", axis=1
            )
            df_tmp["path_sat_np2"] = df_tmp.apply(
                lambda x: f"{data_folder}/satellite/{city}/{x.sat_np2}", axis=1
            )
            df_tmp["path_sat_np3"] = df_tmp.apply(
                lambda x: f"{data_folder}/satellite/{city}/{x.sat_np3}", axis=1
            )
